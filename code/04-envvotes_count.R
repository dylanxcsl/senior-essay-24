...
# 
count_all <- count(filter(rollcalls_all, congress > 87), congress, chamber)
count_7 <- count(rollcalls, congress, chamber)
count_merge_all <- left_join(count_all, count_7, by = c("congress", "chamber")) |>
  mutate(prop = n.y / n.x * 100) |>
  select(chamber, congress, prop) |>
  mutate(type = "gen")
roll_count_hz <- roll_count_hz |> select(chamber, congress, prop) |>
  mutate(type = "env")
count_merge <- rbind(roll_count_hz, count_merge_all)

# STEP 3. Format data into vertical format
...
# STEP 4. Create table of mean vote margins
