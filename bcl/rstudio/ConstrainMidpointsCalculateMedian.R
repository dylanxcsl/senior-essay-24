...
setwd("C:/Users/dab/Dropbox/Documents/CSDP/Bateman and Lapinski/CivilRightsConstraints/ReplicationFiles")

load("PooledIdealCivilRightsConstrained.RData")

### Get vote parameters from IDEAL-produced estimates
bp<-beta.info$"Discrimination D1"
ap<-beta.info$Difficulty

priors<-list()
priors$xp	<-0
priors$xpv	<-0.01
temp 		<-cbind(bp, ap)
row.names(temp)<-row.names(beta.info)
...
### Load the agnostic roll call matrix and estimate with IDEAL
rc <- readKH("PooledMatrixCivilRightsAgnostic.dat", desc="Pooled Matrix, Civil Rights")

rc.issue<-dropRollCall(rc,dropList=list(lop=4,legisMin=4))


rc.ideal<-ideal(rc.issue,d=1,priors=priors, maxiter=30000,thin=30,burnin=3000,
		impute=FALSE,normalize=TRUE,store.item=TRUE,verbose=TRUE)

### Save ideal points and vote parameters
...
beta.info<-as.data.frame(rc.i$betabar)
...
outfile1<-paste("PooledIdealCivilRightsConstrainedMidpoints.csv")

outfile2<-paste("PooledVotesCivilRightsConstrainedMidpoints.csv")

write.table(estimates1, file = outfile1, sep = ",", col.names = NA)
write.table(estimates2, file = outfile2, sep = ",", col.names = NA)

wrkspace<-paste("PooledIdealCivilRightsConstrainedMidpoints.RData")
...
output1<-list()
newfinal <-data.frame()
infile <- paste("PooledIdealCivilRightsConstrainedMidpoints.RData", sep="")
load(infile)
#rc.ideal<-rc.i
#rm(rc.i)
...
outfile1<-paste("MidpointsConstrainedHouseCongressLevelMedians", firstcong, "to", lastcong, ".dta", sep="")
write.dta(newfinal, outfile1)
