...
rc <- readKH("PooledMatrixSocialSecurityConstrained.dat", desc="Pooled Matrix, Social Security - Agnostic")

rc.issue<-dropRollCall(rc,dropList=list(lop=3,legisMin=3))
...
outfile1<-paste("PooledIdealSocialSecurityConstrained.dta")

outfile2<-paste("PooledVotesSocialSecurityConstrained.dta")

write.dta(estimates1, file = outfile1)
write.dta(estimates2, file = outfile2)

wrkspace<-paste("PooledIdealSocialSecurityConstrained.RData")
...
infile <- paste("PooledIdealSocialSecurityConstrained.RData", sep="")
load(infile)
all.info$i <-i
...
outfile1<-paste("SocialSecurityCongressLevelConstrainedMedians", firstcong, "to", lastcong, ".dta", sep="")
