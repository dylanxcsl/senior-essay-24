...
rc <- readKH("PooledMatrixCivilRightsConstrained.dat", desc="Pooled Matrix, Civil Rights - Agnostic")
...
outfile1<-paste("PooledIdealCivilRightsConstrained.dta")

outfile2<-paste("PooledVotesCivilRightsConstrained.dta")

write.dta(estimates1, outfile1)
write.dta(estimates2, outfile2)

wrkspace<-paste("PooledIdealCivilRightsConstrained.RData")
...
output1<-list()
newfinal <-data.frame()
infile <- paste("PooledIdealCivilRightsConstrained.RData", sep="")
load(infile)
...
outfile1<-paste("HouseCongressLevelConstrainedMedians", firstcong, "to", lastcong, ".dta", sep="")
