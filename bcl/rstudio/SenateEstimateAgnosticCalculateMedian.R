...
rc <- readKH("SenatePooledMatrixCivilRightsAgnosticNEWEST.dat", desc="Pooled Matrix, Civil Rights - Agnostic")
...
outfile1<-paste("SenatePooledIdealCivilRightsAgnostic.dta")

outfile2<-paste("SenatePooledVotesCivilRightsAgnostic.dta")

write.dta(estimates1, outfile1)
write.dta(estimates2, outfile2)

wrkspace<-paste("SenatePooledIdealCivilRightsAgnostic.RData")
...
output1<-list()
newfinal <-data.frame()
infile <- paste("SenatePooledIdealCivilRightsAgnostic.RData", sep="")
load(infile)
...
for(i in cong) {
	...
	congress<-members[members$congress==i & members$chamber==2,]
	...
	sdemcongress<-members[members$congress==i & members$chamber==2 & members$party2==100 & members$south==1,]
	...
	### Republican Median
	gopcongress<-members[members$congress==i & members$chamber==2 & members$party2==200,]
	...
	### Democratic Median
	demcongress<-members[members$congress==i & members$chamber==2 & members$party2==100,]
	...
	ndemcongress<-members[members$congress==i & members$chamber==2 & members$party2==100 & members$south==0,]
...
outfile1<-paste("SenateCongressLevelAgnosticMedians", firstcong, "to", lastcong, ".dta", sep="")
